...
authenticator = stauth.Authenticate(credenciais, "credenciais_Dados", "fsyfus%$67fs76AH7", cookie_expiry_days=30)
...
if dados_usuario:
    ...
    if usuario.admin:
        pg = st.navigation(
            {
            "Home": [st.Page("homepage.py", title="Dados Dourados")],
        ...
        pg = st.navigation(
        {
         "Home": [st.Page("homepage.py", title="Dados Douradoso")],
         "Dashboards": [st.Page("dashboard.py", title="Dashboard"), st.Page("indicadores.py", title="Indicadores")],
